...
for idx in "${!arr_divis[@]}"; do
    ...
    for j in "${arr_opt[@]}"; do
        ...
        obj="$(cut -d'*' -f2 <<<$j)"
        echo "running $binary $formula $options --ca-division=$divis --proof-log=$logfile --seed=4"
        output=`$binary $formula $options --ca-division=$divis --proof-log=$logfile --seed=4 2>&1`
