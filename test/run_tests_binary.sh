...
for idx in "${!arr_lazy[@]}"; do
    ...
    for j in "${arr_opt[@]}"; do
        ...
        obj="$(cut -d'*' -f2 <<<$j)"
        echo "running $binary $formula $options --cg-encoding=$lazy --proof-log=$logfile --seed=6"
        output=`$binary $formula $options --cg-encoding=$lazy --proof-log=$logfile --seed=6 2>&1`
